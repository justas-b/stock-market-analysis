...
def display_basic_company(company):
    ...
    column_styles = ["blue1", "blue1", "dark_goldenrod", "deep_pink4", 
                    "light_steel_blue"]
    ...
    stock_table.add_row(company["1. symbol"], company["2. name"], 
                        company["3. type"], company["4. region"],
                        company["8. currency"])
...
def display_detailed_company(company):
    ...
    column_styles = ["blue1", "blue1", "dark_goldenrod", "deep_pink4", 
                    "light_steel_blue", "green4", "red", "green_yellow"]
    table_columns = ["Symbol", "Name", "Exchange", "Country", "Sector",
                    "Year High", "Year Low", "Divident/Share"]
    ...
    if detailed_company:
        ...
        stock_table.add_row(detailed_company["Symbol"], 
                            detailed_company["Name"],
                            detailed_company["Exchange"], 
                            detailed_company["Country"], 
                            detailed_company["Sector"], 
                            detailed_company["52WeekHigh"], 
                            detailed_company["52WeekLow"], 
                            detailed_company["DividendPerShare"])
        console.print(stock_table)
    else:
    ...
    if len(matches):
        if len(matches) == 1:
            ...
            return [match for match in matches
                    if match["1. symbol"] == user_choice][0]
    else:
        console.print(f"{term} not found!")
        return 0


def history_analysis(symbol, option):
    if option == "Monthly":
        api_function = "TIME_SERIES_MONTHLY"
        records_column = "Monthly Time Series"
    elif option == "Weekly":
        api_function = "TIME_SERIES_WEEKLY"
        records_column = "Weekly Time Series"
        
    response = requests.get(API_URL + f"function={api_function}&symbol={symbol}&apikey={API_KEY}")
    records = response.json()[records_column]

    history_table = Table(title = f"{symbol} {records_column}")
    ...
    for date in list(records.keys())[0:10]:
        ...
        history_table.add_row(date, str(open_price), str(close_price), 
                            str(price_difference), str(percentage_change))
...
def display_information(user_input, stock):
    match user_input:
        ...
        case "Monthly Performance":
            return history_analysis(stock["1. symbol"], "Monthly")
        case "Weekly Performance":
            return history_analysis(stock["1. symbol"], "Weekly")
